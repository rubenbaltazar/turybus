...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'mantenimiento.Revision'), OBJECT_ID(N'produccion.Lugar_Interes'), OBJECT_ID(N'produccion.Servicio_Diario'), OBJECT_ID(N'ventas.Billete'))
               AND [type] IN (N'F', N'C')
